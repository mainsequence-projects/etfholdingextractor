...
def _migration_metadata():
    ...
    # AssetTable is included only so the canonical asset_identifier ->
    # AssetTable.unique_identifier foreign key resolves inside the scoped
    # metadata copy; it is NOT part of this provider's registration scope
    # (etfhextractor_metatable_provider_models stays project tables only).
    return metadata_for_models([*etfhextractor_metatable_provider_models(), AssetTable])


# Scoped to the project-owned tables (+ FK targets), so Alembic autogenerate
# never diffs the rest of the built-in ms-markets graph.
