...
def publish_etf_tracking_portfolio(
    *,
    ...
    # Portfolio identity through the typed rows; PortfoliosDataNode uses the
    # provided target_portfolio/portfolio_index instead of an external resolver.
    portfolio_identifier = build_etf_tracker_unique_identifier(normalized_ticker)
    IndexType.upsert(index_type=INDEX_TYPE_EQUITY)
    ...
    node.portfolio_index = portfolio_index
